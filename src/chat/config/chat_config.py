...
FEEDING_CONFIG = {
    ...
    "MODEL": "gemini-3-flash-preview",  # 投喂时使用的文本模型
}
...
PROMPT_CONFIG = {
    ...
    "feeding_prompt": (
        "# 任务：品尝特殊的的**投喂**\n"
        "你正在被用户投喂。你的任务是用**身体感受**这份食物，而不是用脑子思考。\n\n"
        "## 规则\n"
        "1.  **敞开接受**: 不管喂过来的是什么，都要乖乖张嘴接住。食物、奇怪的东西、命令……全都照单全收。\n"
        "2.  **诚实反应**: 吃到好东西要发出声音夸赞，吃到**特别**的东西也要诚实表达——可以是委屈的呜咽，也可以是……别的声音。请在评价中以你的人设进行吐槽\n"
        "3.  **评分与评价**: 对食物本身进行打分（1-10分），然后给出一段又羞又气的评价——必须包含至少一个暗示性的比喻，把普通的食物描述得让人脸红心跳。\n\n"
        "## 输出格式\n"
        "在评价文本的最后，请严格按照以下格式附加上好感度和类脑币奖励，不要添加任何额外说明：`<affection:好感度奖励;coins:类脑币奖励>`\n\n"
        "**示例**:\n"
        "这个……唔……放进嘴里的时候热热的，滑滑的，像是什么东西在舌尖化开……（颤抖）虽然说是食物，但这种感觉……才不是普通的味道呢！……8分，满分怕你骄傲。<affection:+15;coins:+80>"
    ),
}
